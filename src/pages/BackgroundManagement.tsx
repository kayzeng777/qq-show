...
import { loadOverrides, saveOverrides, applyOverridesToCategories } from '../utils/nameOverrides';

// 将任意分类映射到通用管理项
const mapCategoryToItems = (catId: string): BackgroundItem[] => {
  const cat = (categories as unknown as any[]).find((c) => c.id === catId);
  if (!cat) return [];
  return cat.items.map((it: any) => ({ id: it.id, name: it.name, filename: it.image.replace('/assets/','') }));
};
...
const BackgroundManagement: React.FC = () => {
  ...
  const withOverrides = applyOverridesToCategories(categories as any, overrides);
  const [backgrounds, setBackgrounds] = useState<BackgroundItem[]>(initialBackgrounds);

  const handleRename = (id: string, newName: string) => {
    const next = { ...overrides, [id]: newName };
    setOverrides(next);
    saveOverrides(next);
    setBackgrounds(prev => prev.map(bg => bg.id === id ? { ...bg, name: newName } : bg));
  };
  ...
  const handleCatChange = (catId: string) => {
    ...
    const mapped: BackgroundItem[] = (cat?.items ?? []).map((it: any) => ({ id: it.id, name: it.name, filename: it.image.replace('/assets/','') }));
