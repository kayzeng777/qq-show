...
// 检查重复背景的函数
export const checkDuplicateBackgrounds = (_items: BackgroundItem[]): BackgroundItem[][] => {
  // 这里需要实现重复检查逻辑
  // 由于无法直接比较图片内容，需要手动检查
  return [];
};
...
// 重新命名背景的函数
export const renameBackground = (item: BackgroundItem, newName: string): BackgroundItem => {
  return {
    ...item,
    name: newName,
  };
